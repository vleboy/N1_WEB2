  ...
  <div class="sbAll">
    <div class="nowList">
      ...
      <Table :columns="columns1" :data="user" size="small" ref="table_0">
        ...
        <template slot-scope="{row, index}" slot="trueWinloseAmount">
          <span :style="{color:gameWinloseAmount(row, '1120000').color}">{{gameWinloseAmount(row, '1120000').winloseAmount}}</span>
        </template>
        <template slot-scope="{row, index}" slot="trueSubmitAmount">
          <span>{{gameSubmitAmount(row, '1120000')}}</span>
        </template>
        <template slot-scope="{row, index}" slot="videoWinloseAmount">
          <span :style="{color:gameWinloseAmount(row, '1080000').color}">{{gameWinloseAmount(row, '1080000').winloseAmount}}</span>
        </template>
        <template slot-scope="{row, index}" slot="videoSubmitAmount">
          <span>{{gameSubmitAmount(row, '1080000')}}</span>
        </template>
      </Table>
    </div>
    <div class="childList">
    ...
    <div class="playerList" id="playerList">
      ...
      <Table :columns="columns2" :data="playerList" size="small" ref="table_2">
        ...
        <template slot-scope="{row, index}" slot="pVideoWinloseAmount">
          <span :style="{color:gameWinloseAmount(row, '1080000').color}">{{gameWinloseAmount(row, '1080000').winloseAmount}}</span>
        </template>
      </Table>
    </div>
  ...
  data() {
    return {
      ...
      columns1: [
        ...
        {
          title: "SB电子游戏(输赢金额)",
          slot: "videoWinloseAmount"
        },
        {
          title: "SB电子游戏(商家交公司)",
          slot: "videoSubmitAmount"
        }
      ],
      columns2: [
        {
